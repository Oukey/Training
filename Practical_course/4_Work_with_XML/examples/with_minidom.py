# Пример парсинга xml с помощью minidom
# Это не очень хорошее решение - годится только для ознакомления
    ...
    def getXml(self, url):
        ...
        if node.nodeType == xml.dom.Node.ELEMENT_NODE:
            ...
            for (name, value) in node.attributes.items():
                print(' Arrt -- имя: %s значение: %s' % (name, value))
        return node

    def handleXml(self, xml):
        rem = xml.getElementsByTagName('zAppointments')
        appointments = xml.getElementsByTagName("appointmant")
    ...
    def handleAppt(self, appt):
        ...
        if self.flag == 'file':
            try:
                ...
                alarm = self.getElement(appt.getElementsByTagName("alar,Time")[0])
